...
/**
 * WooCommerce gateway: redirect to Fintoc-hosted checkout.
 */
final class Gateway_Fintoc extends \WC_Payment_Gateway
{
	 ...
	 */
	private function failure_payment_result(): array
	{
		return [
			'result'   => 'failure',
			'redirect' => '',
	 ...
	 */
	private function log_checkout_session_failure(string $context, array $response): void
	{
	...
	public function process_payment($order_id)
	{
		...
		// Fintoc metadata: string / number / boolean only; keep values simple.
		$metadata = [
		...
		// Direct deposit: send recipient only when configured in WordPress (see Options::sends_recipient_from_wordpress).
		if (Options::sends_recipient_from_wordpress()) {
			$o         = Options::get_all();
			$recipient = [
	...
	{
		if (!defined('PWL_FINTOC_EDITION') || PWL_FINTOC_EDITION !== 'pro') {
			return new \WP_Error('pwl_fintoc_refund', __('Refunds require Fintoc Pro.', 'pwl-integracion-fintoc'));
		}
		...
		$pi = (string) $order->get_meta('_pwl_fintoc_payment_intent_id');
		if ($pi === '') {
			return new \WP_Error('pwl_fintoc_refund', __('No Fintoc payment intent on this order.', 'pwl-integracion-fintoc'));
		}

		$currency = strtoupper($order->get_currency());
		$minor    = null;
		if (null !== $amount && $amount > 0) {
			$minor = self::minor_from_float((float) $amount, $currency);
		}

		$res = Client::create_refund($pi, $minor);
		if (!$res['ok']) {
			$err = is_array($res['data']) && isset($res['data']['error_message'])
				? (string) $res['data']['error_message']
				: __('Refund request failed.', 'pwl-integracion-fintoc');

			return new \WP_Error('pwl_fintoc_refund', $err);
		}

		$order->add_order_note(
			sprintf(
				/* translators: %1$s: reason */
				__('Fintoc refund requested. Reason: %1$s', 'pwl-integracion-fintoc'),
				$reason !== '' ? $reason : '—',
			),
		);
	...
	private static function minor_from_float(float $amount, string $currency): int
	{
		if ($currency === 'CLP') {
			return (int) round($amount);
		}

		// MXN: peso has centavos — Woo stores decimal.
